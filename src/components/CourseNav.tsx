 ...
 *
 * El orden es el del colegio (801…806, 901…905, 1001…1004, 1101…1104), que es
 * el orden numérico del código, y da la vuelta: del último se pasa al primero.
 * Solo entran los cursos que existen en la base — navegar a uno sin importar
 * dejaría la página cargando para siempre.
 *
...
export function CourseNav({ code, ciclo }: Props) {
  ...
  const { prev, next, pos, total } = useMemo(() => {
    const existentes = sortedCourseCodes(courses);
